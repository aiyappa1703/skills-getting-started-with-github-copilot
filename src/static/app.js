          ...
          btn.addEventListener('click', async (event) => {
            ...
            try {
              const response = await fetch(
                `/activities/${encodeURIComponent(activityName)}/signup?email=${encodeURIComponent(emailToDelete)}`,
                { method: 'DELETE' }
              );
              ...
              if (response.ok) {
                ...
                fetchActivities();
                messageDiv.textContent = `Unregistered ${emailToDelete} from ${activityName}`;
